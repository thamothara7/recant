#!/usr/bin/env bash
# Start the four Recant services together in one terminal. Ctrl+C stops all.
#
#   bash ops/run-services.sh
#
# gateway :8000  quarantine :8001  forensics :8002  guard :8003
set -euo pipefail
...
BUSY=$(lsof -nP -ti :8000 -ti :8001 -ti :8002 -ti :8003 2>/dev/null || true)
if [ -n "$BUSY" ]; then
    echo "ports 8000/8001/8002/8003 are already in use (pids: $(echo "$BUSY" | tr '\n' ' '))." >&2
...
uv run uvicorn services.guard.app:app          --port 8003 & pids+=("$!")

trap 'kill "${pids[@]}" 2>/dev/null || true' INT TERM
echo "gateway :8000  quarantine :8001  forensics :8002  guard :8003   (Ctrl+C to stop all)"
wait
